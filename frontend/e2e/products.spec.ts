...
test.describe('Products', () => {
  ...
  test('should open and close add product form', async ({ page }) => {
    ...
    await expect(page.locator('text=New Product')).toBeVisible({ timeout: 5000 });
  ...
  test('should show pagination or empty/data state', async ({ page }) => {
    ...
    const showingText = await page.locator('text=Showing').isVisible().catch(() => false);
    const noDataText  = await page.locator('text=No data found').isVisible().catch(() => false);
