...
export default function ProductsPage() {
  ...
  return (
    <div>
        ...
        <div className="bg-white p-6 rounded-xl border mb-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('title')} className="w-full border rounded-lg px-3 py-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">SKU *</label>
              <input {...register('sku')} className="w-full border rounded-lg px-3 py-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Price *</label>
              <input {...register('price')} type="number" step="0.01" className="w-full border rounded-lg px-3 py-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Stock *</label>
              <input {...register('stockQuantity')} type="number" className="w-full border rounded-lg px-3 py-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Category *</label>
              <select {...register('categoryId')} className="w-full border rounded-lg px-3 py-2" required>
            ...
            <div>
              ...
              <input {...register('description')} className="w-full border rounded-lg px-3 py-2" />
            </div>
      ...
      <DataTable data={products} columns={columns} onEdit={handleEdit}
        onDelete={(row) => { if (confirm(`Delete "${row.title}"?`)) deleteMutation.mutate(row.id); }}
        isLoading={isLoading} />
    </div>
  );
}