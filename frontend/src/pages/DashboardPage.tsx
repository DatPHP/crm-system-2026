  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((card) => (
          <div key={card.label} className="bg-white rounded-xl p-5 border shadow-sm flex items-center gap-4">
            <div className={`${card.color} p-3 rounded-lg text-white`}>
              <card.icon size={24} />
            </div>
            <div>
              <p className="text-sm text-gray-500">{card.label}</p>
              <p className="text-2xl font-bold truncate" title={String(card.value)}>{card.value}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            <tbody>
              {data.recentOrders.map((order: any) => (
                <tr key={order.id} className="border-b last:border-0">
                  ...
                  <td className="py-2">${Number(order.totalPrice).toLocaleString()}</td>
                  <td className="py-2">
          ...
          <div className="space-y-3">
            {data.ordersByStatus.map((item: any) => (
              <div key={item.status} className="flex items-center justify-between">
                ...
                <span className="font-bold text-gray-700">{item._count.status}</span>
              </div>
            ))}
          </div>
        </div>
